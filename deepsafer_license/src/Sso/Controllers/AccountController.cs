...
public class AccountController : Controller
{
    ...
    public async Task<IActionResult> ExternalCallback()
    {
        ...
        // They could have an existing Deepsafer account in the User table though.
        if (user == null)
        {
        ...
        // Either way, we have associated the SSO login with a Deepsafer user.
        // We will now sign the Deepsafer user in.
        if (user != null)
        {
    ...
    /// <summary>
    /// Attempts to map the external identity to a Deepsafer user, through the SsoUser table, which holds the `externalId`.
    ...
    /// <summary>
    /// Provision an SSO-linked Deepsafer user.
        ...
        IEnumerable<Claim> claims, string userIdentifier, SsoConfigurationData config)
    {
        ...
        if (existingUser != null)
        {
            ...
            // If the user already exists in Deepsafer, we require that the user already be in the org,
