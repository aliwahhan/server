...
public class HandlebarsMailService : IMailService
{
    ...
    public async Task SendOrganizationInviteEmailsAsync(OrganizationInvitesInfo orgInvitesInfo)
    {
        ...
        MailQueueMessage CreateMessage(string email, OrganizationUserInvitedViewModel model)
        {
            ...
            {
                { IsFreeOrg: true, OrgUserHasExistingUser: true } => "You have been invited to a Deepsafer Organization",
                { IsFreeOrg: true, OrgUserHasExistingUser: false } => "You have been invited to Deepsafer Password Manager",
                { IsFreeOrg: false, OrgUserHasExistingUser: true } => $"{model.OrganizationName} invited you to their Deepsafer organization",
                { IsFreeOrg: false, OrgUserHasExistingUser: false } => $"{model.OrganizationName} set up a Deepsafer account for you"
            };
